...
export default function TagEditor({ tags, onChange, max = 10, sanitize = sanitizeTags }) {
  ...
  function add() {
    ...
    if (tags.length >= max && !tags.includes(candidate)) { setError(`You can add up to ${max} tags.`); return; }
    onChange(sanitize([...tags, candidate])); setDraft(""); setError("");
  }
